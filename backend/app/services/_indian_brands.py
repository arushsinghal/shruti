...
_TSV_PATH = (
    ...
    / "data" / "ontology" / "indian_brands.tsv"
)

def _load_tsv() -> dict[str, str]:
    """Load brand→generic mappings from the ontology TSV at import time.

    Returns an empty dict silently if the file is missing (tests, bare installs).
    """
    mapping: dict[str, str] = {}
    if not _TSV_PATH.exists():
        _log.debug("indian_brands.tsv not found at %s — using hardcoded dict only", _TSV_PATH)
        return mapping
    try:
        with open(_TSV_PATH, encoding="utf-8") as fh:
            for line in fh:
                parts = line.strip().split("\t")
                if len(parts) >= 2:
                    mapping[parts[0].strip().lower()] = parts[1].strip().lower()
        _log.debug("indian_brands.tsv: loaded %d entries", len(mapping))
    except Exception as exc:
        _log.warning("Could not load indian_brands.tsv: %s", exc)
    return mapping

# TSV entries (280 entries) are the primary source.
# _BRAND_TO_GENERIC (126 entries from Claude Science) fills any gaps not in the TSV.
_TSV_BRANDS: dict[str, str] = _load_tsv()
_MERGED_BRANDS: dict[str, str] = {**_BRAND_TO_GENERIC, **_TSV_BRANDS}  # TSV wins on conflict


def resolve_to_generics(name: str) -> list[str]:
    """Return generic component(s) for a brand name, or [name.lower()] if unknown.

    Lookup order:
      1. _FDC_COMPONENTS — structured component list (Claude Science, enables per-component DDI check)
      2. _MERGED_BRANDS  — flat brand→generic string (TSV primary, hardcoded fallback)

    FDCs return all components so hidden ingredients are each checked against
    interaction rules. A name already known as a generic passes through unchanged.
    """
    lower = name.strip().lower()
    if lower in _FDC_COMPONENTS:
        return list(_FDC_COMPONENTS[lower])
    if lower in _MERGED_BRANDS:
        mapped = _MERGED_BRANDS[lower]
        if "+" in mapped:
            return [c.strip() for c in mapped.split("+")]
        return [mapped]
    return [lower]


def expand_med_names(med_names: list[str]) -> list[str]:
    """Expand brand names to include all generic equivalents for DDI matching.

    Original names are kept so allergy and display logic is unaffected.
    Deduplicates while preserving order.
    """
    seen: set[str] = set()
    result: list[str] = []
    for name in med_names:
        for resolved in [name, *resolve_to_generics(name)]:
            key = resolved.lower()
            if key not in seen:
                seen.add(key)
