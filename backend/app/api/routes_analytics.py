...
async def get_analytics_dashboard():
    ...
    async with db_connect() as db:
        ...
        async with db.execute(
            ...
            row = await cur.fetchone()
            sessions_this_week = row["cnt"] if row else 0
        ...
        async with db.execute(
            ...
            row = await cur.fetchone()
            completed_sessions = row["cnt"] if row else 0
        ...
        async with db.execute(
            ...
            row = await cur.fetchone()
            cloud_count = int(row["cloud"] or 0)
            edge_count = int(row["edge"] or 0)
        
        # 5. Aggregate top symptoms, vitals, medications, allergies across all clinical_facts
        ...
        # 6. Sessions per day (last 7 days)
        sessions_by_day = []
        for day_offset in range(6, -1, -1):
            ...
            async with db.execute(
                ...
                sessions_by_day.append({
                    "date": day.strftime("%a"),
                    "consultations": row["cnt"] if row else 0
                })
