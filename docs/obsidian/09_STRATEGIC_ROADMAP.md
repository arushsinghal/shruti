...
3. **Learning flywheel** — doctor corrections → Bayesian confidence → auto-promoted at 0.9 + 3 clinics + 3 confirmations → all doctors benefit. **Verified 2026-07-03: NOW OPERATIONAL.** As of 2026-07-02/03, `routes_fact_review.py` (accept/edit/reject/add) and `routes_notes.py`'s SOAP hallucination flag all call `record_correction()`/`record_false_positive()`. The write path that was previously confirmed dead (see `16_FULL_APP_REVIEW.md` F2 history) is now closed. What is still NOT built: the richer `LearningEvent` schema with structured reason/scope categories described in `10_CONTINUAL_LEARNING_SYSTEM.md` — the current write path is simpler than that eventual design. Dynamic per-clinic drug alias learning (Block 2.2 in `14_BUILD_PLAN.md`, "Telma" → telmisartan learned from a specific doctor's correction) is also still not built — current brand-name recognition is static (`_indian_brands.py`, 334 entries), not learned per-clinic. Safe to claim "the flywheel is wired and every doctor correction feeds it" externally now; not yet safe to claim per-clinic dynamic vocabulary learning.
