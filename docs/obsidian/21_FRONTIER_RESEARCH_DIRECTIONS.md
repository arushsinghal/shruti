...
**Base model candidates, researched 2026-07-03 (desk research — not live-tested, no hosting/credentials available to verify these directly, unlike Sarvam which was live-verified):**

No existing open-weight model is simultaneously (a) a top general-intelligence performer, (b) medical-specific, and (c) Indic/Hinglish-fluent. That combination has to be built, not downloaded — which is the actual argument for this direction existing at all, not a reason to wait for someone else to ship it.

- **Sarvam-M** — open-weight, already Indic-tuned, self-hostable (eliminates per-token API cost at scale). Not medical-specific. Remains the strongest starting point specifically *because* Hinglish fluency is this product's real differentiator, and no medical-tuned model found has that.
- **Meditron-FO ("Fully Open Meditron")** — a clinically-focused model that is open across weights, training data, *and* training code, evaluated on real clinical benchmarks (MedMCQA, PubMedQA). The "fully open, fully auditable" framing is a direct philosophical match to this product's own "no proof, no fact" discipline — worth a real look as a medical-grounding base or comparison point, despite having no Indic tuning.
- **MedGemma** (Google's medical fine-tune of Gemma) — more mainstream tooling/support than Meditron-FO, same gap: no Indic tuning.
- **General frontier open-weight leaders (GLM-5.2, DeepSeek V4 Pro, as of 2026-07)** — lead open-weight benchmarks on raw intelligence, but are large enough that self-hosting cost/complexity is likely impractical at this company's current stage, and neither is medical-specific or Indic-tuned. Not a fit for this direction; noted here so the option isn't silently missed later, not as a recommendation.

**Working recommendation, not a decision:** Sarvam-M stays the lead candidate for the reason above, but Meditron-FO's training corpus and its "fully open pipeline" framing are worth studying before committing — either as a source of medical training signal to blend into a Sarvam-M fine-tune, or as a direct comparison baseline once real fine-tuning work starts. This is the concrete instance of [[10_CONTINUAL_LEARNING_SYSTEM]]'s "Stage 3: Distillation" — the teacher is the whole experienced Lipi system (deterministic extractors, doctor corrections, CDS alert history), the student is a fine-tuned base model (candidate above) serving both consumers named above.
