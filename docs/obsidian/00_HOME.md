...
- [[22_OPUS_FRONTIER_RESEARCH]] - Opus's frontier R&D proposal: a research lab *inside* the product company, aimed at (1) solving hard Indian healthcare problems and (2) piping that R&D straight into features doctors use. Spine is the Ankit Gupta "AI Personalized Medicine" RFS — Lipi as India's agent harness for personalized primary care. Tracks: A personalized prescribing (sequencing-free pharmacogenomics), B early detection at first contact (TB etc.), C precision antibiotic use / AMR, D the trust substrate (Hinglish understanding, certified grounding, correction economics). Every track terminates in a doctor-gated feature; publication is a byproduct. Pushes back on parts of [[21_FRONTIER_RESEARCH_DIRECTIONS]] and includes a Claude Science R&D-for-product playbook. Proposal only.
