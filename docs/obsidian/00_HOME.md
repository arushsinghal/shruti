...
- [[27_CLINICAL_MEMORY_ASSISTANT_SPEC]] - Build spec for the "Jarvis" clinical memory assistant: a per-doctor RAG panel over confirmed consultation history. Leads with a hard compliance constraint (ABDM Health Data Management Policy requires India-only data storage — verified against current law 2026-07-03) that rules out any foreign-hosted LLM for this feature. Data model, retrieval architecture, safety boundaries, and UI spec included. Not yet built.
