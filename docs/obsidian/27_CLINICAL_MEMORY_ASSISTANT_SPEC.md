...
**What this requires:**
- **Inference:** Sarvam's LLM offering (same vendor already used for ASR, Indian company, on-shore) as the default. If capability is insufficient, fall back to a self-hosted open-weight model (Llama / Qwen / DeepSeek / Gemma class) on India-region cloud compute (AWS `ap-south-1` Mumbai, Azure Central India) — never a foreign SaaS inference endpoint.
  - **Researched 2026-07-03 (desk research, not live-tested):** Vertex AI's Gemini API is available in `asia-south1` (Mumbai) as a real in-region inference option, but developer-forum reports indicate only Gemini 2.5 Flash is confirmed there, not the flagship Pro tier — worth confirming directly with Google before relying on it. A cleaner compliance story: self-host **Gemma** (Google's open-weight family, Apache 2.0) on AWS `ap-south-1`, confirmed to have real GPU capacity there (G5, G6, P4d, P5 instance families, up to H100-class) — since you control the infrastructure directly, this avoids any dependency on a foreign company's regional data-residency policy holding up under scrutiny. Trade-off: Gemma/Gemini's general capability and context window both likely exceed Sarvam's, but neither has Sarvam's Hinglish/Indic-clinical-speech specialization, which is the actual differentiator for this product's retrieval task.
